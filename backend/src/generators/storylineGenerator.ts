...
export async function generateNextStoryPage(
  ...
  const instructions = [
    ...
    "",
    "Guidance:",
    "- If previousOption is missing, this is the opening scene. Use definition.startHook to grab attention.",
    ...
    "- Use { type: 'branch', options: [...] } only when a choice expands into immediate sub-choices.",
  ...
  const { json } = await generateJsonFromInputs<StoryPage>(
    { definition, previousOption: previousOption ?? null },
    instructions,
  );
  return json;
}
