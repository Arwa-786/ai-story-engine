...
export async function generateHashText(
  ...
  console.log(`\n🎯 Text Agent: Starting hash text generation`);
  
  const normalisedInputs = normaliseInputs(request.inputs);
  console.log(`   Input Keys: ${Object.keys(normalisedInputs).join(', ')}`);
  ...
  console.log(`   Instructions: "${instructions.substring(0, 100)}${instructions.length > 100 ? '...' : ''}"`);
  console.log(`   System Prompt: ${hashAgentConfig.systemPrompt ? 'Configured' : 'None'}`);
  console.log(`   Model Candidates: ${gatewayModelCandidates.join(', ')}`);

  const attemptLog: Array<{ modelId: string; message: string }> = [];

  for (const modelId of gatewayModelCandidates) {
    console.log(`\n   🔸 Attempting with model: ${modelId}`);
    try {
      ...
      console.log(`   Generated text length: ${rawOutput.length} characters`);
      ...
      console.error(`   ❌ Failed with model ${modelId}: ${message}`);
